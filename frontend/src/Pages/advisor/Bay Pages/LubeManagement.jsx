...
export default function LubeManagement() {
  ...
  const clickedInBtn = async (data) => {
    ...
    try {
      const response = await createBayRecord(setData, "lube");
  ...
  const clickedOutBtn = async () => {
    ...
    console.log("hi", setData);
    try {
      const response = await updateBayRecord(setData, "lube");
      if (response.status === 200) {
        toast.success("Record Add Successfully");
        navigate(0);
      }
  ...
  const getRecordTimeDetails = useCallback(async () => {
    try {
      const response = await getAllBayTypeDetails("lube", {
        record_id: record_id,
      });
      if (response.status === 200) {
  ...
  const getBayDetails = useCallback(async () => {
    try {
      const response = await getAllBayDetails("lube");
      if (response.status === 200) {
