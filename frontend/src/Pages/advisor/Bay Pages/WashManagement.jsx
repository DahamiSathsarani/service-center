...
export default function WashManagement() {
  ...
  const clickedInBtn = async (data) => {
    ...
    try {
      const response = await createBayRecord(setData, "wash");
  ...
  const clickedOutBtn = async () => {
    ...
    console.log("hi", setData);
    try {
      const response = await updateBayRecord(setData, "wash");
      if (response.status === 200) {
        toast.success("Record Add Successfully");
        navigate(0);
      }
  ...
  const getRecordTimeDetails =useCallback(async () => {
    try {
      const response = await getAllBayTypeDetails("wash", {
        record_id: record_id,
      });
      if (response.status === 200) {
  ...
  const getBayDetails = useCallback(async () => {
    try {
      const response = await getAllBayDetails("wash");
      if (response.status === 200) {
